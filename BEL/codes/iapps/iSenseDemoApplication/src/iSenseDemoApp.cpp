...
private:
	...
	UartTimeBuffer buf0, buf1, time_diff;
};
...
//				os().debug("rx 0");
			...
			else
			{
				...
				uint32 tdiff = max(t1, t0) - min(t1, t0);
				time_diff.sec = tdiff / 1000;
				time_diff.ms = tdiff % 1000;;
				synched = true;
//				os().debug("rx 1 Synched = %d Time diff = %d %d", synched, time_diff.sec, time_diff.ms);
			}
		}
		else
		{
