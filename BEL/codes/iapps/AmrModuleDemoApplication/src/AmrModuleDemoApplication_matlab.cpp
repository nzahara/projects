...
#define UPPER_COMPENSATION_TRIGGER 3600
#define LOWER_COMPENSATION_TRIGGER 500
...
execute( void* userdata )
{
	if ((uint32)userdata == TASK_CALIBRATE)
	{
		...
		amr_->calibrate();
		//os().debug("Done calibrating");
		amr_->start();
	}
	if ((uint32)userdata == TASK_ALARM)
	{
		// output text message
		//os().debug("AMR vehicle detected");
	}
}
...
//	Uart& uart = os().uart(0);
//	uart.write_buffer((char*) buf, 108);
	// Here, additional reasoning based upon the buffer data could be done
	static uint16 ctr = 0;
	uint16 outbuffer[2];
	outbuffer[0] = 0x2020;
	outbuffer[1] = ctr;
	os().radio().send(ISENSE_RADIO_BROADCAST_ADDR, 108, (uint8*) buf, Radio::ISENSE_RADIO_TX_OPTION_NONE, this);
//	os().radio().send(ISENSE_RADIO_BROADCAST_ADDR, 4, (uint8*) outbuffer, Radio::ISENSE_RADIO_TX_OPTION_ACK, this);
	os().debug("%d %d", ctr++, buf->sec);
...
//			char s[5] = "0000";
//			snprintf(s, 4, "%d", buf->buf[2*i]);
//			uart.write_buffer(s, 5);

//			snprintf(s, 4, "%d", buf->buf[2*i+1]);
//			uart.write_buffer(s, 5);
...
////		{
////			os().debug("Received: %d from %x at %d sec",  rbuf->buf[2*i], src_addr, rbuf->sec);
////		}
////		os().debug("Received: %d from %x",  *(uint16*)(buf+2), src_addr);
//	}
