...
def forward_checking_with_heuristic_mrv(failed_index, state_domain_dict, neighbor_state_dict, is_backtrack=False, parent_domain_to_be_chosen=None):
    """
    ...
    if failed:
        ...
        if failed_index > 0:
            visited_node.clear()
            forward_checking_with_heuristic_mrv(failed_index, state_domain_dict, neighbor_state_dict, True, None)
        else:
            ...
            if len(state_domain_dict_list) == 0:
                return
            read_csv()
            forward_checking_with_heuristic_mrv(0, state_domain_dict, neighbor_state_dict, True, state_domain_dict_list[0])
    else:
        pass
...
def minimum_remaining_value(state_domain_dict, neighbor_state_dict, visited_node, key):
    ...
    """
    min_constraint_key = ""
    min_constraint_len = sys.maxsize #len(state_domain_dict[key]) #sys.maxsize#
...
def main():
    ...
    forward_checking_with_heuristic_mrv(0, state_domain_dict, neighbor_state_dict)
