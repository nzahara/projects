...
def forward_checking(failed_index, state_domain_dict, neighbor_state_dict, is_backtrack=False, parent_domain_to_be_chosen=None):
    """
    ...
    if failed:
        ...
        if failed_index > 0:
            visited_node.clear()
            forward_checking(failed_index, state_domain_dict, neighbor_state_dict, True, None)
        else:
            ...
            if len(state_domain_dict_list) == 0:
                return
            read_csv()
            forward_checking(0, state_domain_dict, neighbor_state_dict, True, state_domain_dict_list[0])
    else:
        pass
...
def main(map_name=None):
    ...
    forward_checking(0, state_domain_dict, neighbor_state_dict)
